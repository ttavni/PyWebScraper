...
def dataframe_to_flare(df, page_urls, domain):
	...
	for line in df1.values:
		...
		if not category in category_list:
			d['children'].append({"name": category, "children": [
				{"name": sub_category, "children": [{"name": sub_category_type, "size": count}]}]})
		...
		# if 'category' IS in category_list, add a new child to it
			...
			if not sub_category in sub_list:
				d['children'][category_list.index(category)]['children'].append(
					{"name": sub_category, "children": [{"name": sub_category_type, "size": count}]})
			else:
				d['children'][category_list.index(category)]['children'][sub_list.index(sub_category)][
					'children'].append({"name": sub_category_type, "size": count})
